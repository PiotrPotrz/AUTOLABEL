...
#
#         def display():
#             self.idx = random.randint(0, len(self.pil_images) - 1)
#             self.tk_image = ImageTk.PhotoImage(self.pil_images[self.idx])
#             label_photo.config(image=self.tk_image)
#
#         def get_input():
...
#         self.pil_images = self.__load()
#         self.dino = PredictWithDino()
#         self.idx = 0  # Aktualny indeks obrazów
#         self.tk_image = None  # Referencja do aktualnego obrazu PhotoImage
#
#     def previewer(self):
#         text = ""
#
...
#                     label, score, x_min, y_min, x_max, y_max = row
#                     cv2.rectangle(opencv_image, (int(x_min), int(y_min)), (int(x_max), int(y_max)), (0, 0, 255), 2)
#                     cv2.putText(opencv_image, label, (int(x_min), int(y_max)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
#                 rgb_image = cv2.cvtColor(opencv_image, cv2.COLOR_BGR2RGB)
#                 pil_image = Image.fromarray(rgb_image)
#                 self.tk_image = ImageTk.PhotoImage(pil_image)  # Zachowaj referencję do obrazu
#                 label_photo.config(image=self.tk_image)
#
#         def display():
#             self.idx = random.randint(0, len(self.pil_images) - 1)  # Wybierz losowy obraz
#             self.tk_image = ImageTk.PhotoImage(self.pil_images[self.idx])  # Zachowaj referencję do obrazu
#             label_photo.config(image=self.tk_image)
#
#         def get_input():
#             nonlocal text  # Użycie nonlocal zamiast global
...
#
#         # Wyświetl pierwszy obraz
#         self.tk_image = ImageTk.PhotoImage(self.pil_images[0])  # Zachowaj referencję do pierwszego obrazu
#         label_photo = Label(root, image=self.tk_image)
#         label_photo.grid(row=0, column=1, sticky="nsew", padx=5, pady=5)
#
#         box_var = tk.DoubleVar()
...
#         self.dino = PredictWithDino()
#         self.idx = 0  # Aktualny indeks obrazów
#         self.tk_images = []  # Lista przechowująca obiekty PhotoImage
#
#     def previewer(self):
#         text = ""
#
...
#                 rgb_image = cv2.cvtColor(opencv_image, cv2.COLOR_BGR2RGB)
#                 pil_image = Image.fromarray(rgb_image)
#                 tk_image = ImageTk.PhotoImage(pil_image)  # Utwórz nowy obiekt PhotoImage
#                 self.tk_images.append(tk_image)  # Dodaj obiekt do listy, aby zachować referencję
#                 label_photo.config(image=tk_image)
#
#         def display():
#             self.idx = random.randint(0, len(self.pil_images) - 1)  # Wybierz losowy obraz
#             tk_image = ImageTk.PhotoImage(self.pil_images[self.idx])  # Utwórz nowy obiekt PhotoImage
#             self.tk_images.append(tk_image)  # Dodaj obiekt do listy, aby zachować referencję
#             label_photo.config(image=tk_image)
#
#         def get_input():
...
#
#         # Wyświetl pierwszy obraz
#         tk_image = ImageTk.PhotoImage(self.pil_images[0])  # Utwórz pierwszy obiekt PhotoImage
#         self.tk_images.append(tk_image)  # Dodaj obiekt do listy, aby zachować referencję
